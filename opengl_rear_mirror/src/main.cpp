...
int main() {
	...
	std::filesystem::path screenShaderVertPath = projectRoot/"res"/"shaders"/"screen_shader.vert";
	std::filesystem::path screenShaderFragPath = projectRoot/"res"/"shaders"/"screen_shader.frag";
	Shader screenShader(screenShaderVertPath.string().c_str(), screenShaderFragPath.string().c_str());
    ...
    std::filesystem::path cubeTexturePath = projectRoot/"res"/"container.jpg";
    unsigned int cubeTexture = loadTexture(cubeTexturePath.string().c_str());
    std::filesystem::path planeTexturePath = projectRoot/"res"/"wall.jpg";
    unsigned int planeTexture = loadTexture(planeTexturePath.string().c_str());
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// note that we disable pitch constrains for this specific case (otherwise we can't reverse camera's pitch values)
