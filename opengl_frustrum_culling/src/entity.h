...
// 	virtual bool isOnFrustrum(const Frustrum& camFrustrum, const Transform& transform) const = 0;
// 	virtual bool isOnOrForwardPlane(const Plane& plane) const = 0;

// 	bool isOnFrustrum(const Frustrum& camFrustrum) const {
// 		return (
// 			isOnOrForwardPlane(camFrustrum.leftFace) &&
// 			isOnOrForwardPlane(camFrustrum.rightFace) &&
// 			isOnOrForwardPlane(camFrustrum.topFace) &&
// 			isOnOrForwardPlane(camFrustrum.bottomFace) &&
...
// 	bool isOnFrustrum(const Frustrum camFrustrum, const Transform& transform) const final {
...
// 		return (
// 			globalSphere.isOnOrForwardPlane(camFrustrum.leftFace) &&
...
// 	glm::ve3 center{0.0f, 0.0f, 0.0f};
// 	float extend{0.0f};

// 	SquareAABB(const glm::vec3& inCenter, float inExtent) 
// 		: BoundingVolume{}, center{inCenter}, extend{inExtent}
// 	{}
...
// 		const glm::vec3 right = tranform.getRight() * extent;
...
// 		const flaot newIk = std::abs(glm::dot(glm::vec3{0.0f, 0.0f, 1.0f}, right)) +
...
// 		return (
// 			globalAABB.isOnOrForwardPlane(camFrustrum.leftFace) &&
// 			globalAABB.isOnOrForwardPlane(camFrustrum.rightFace) &&
// 			globalAABB.isOnOrForwardPlane(camFrustrum.topFace) &&
// 			globalAABB.isOnOrForwardPlane(camFrustrum.bottomFace) &&
// 			globalAABB.isOnOrForwardPlane(camFrustrum.nearFace) &&
// 			globalAABB.isOnOrForwardPlane(camFrustrum.farFace)
// 		);
// 	);
// };
...
// 	AABB(const glm::vec3& inCenter, floatiI, float iJ, float iK)
...
// 	bool isOnFrustrum(const Frustrum& camFrustrum, const Transform& transform) const final {
// 		// get global scale thanks to transform
// 		const glm::vec3 globalCenter{transform.getModelMatrix() * glm::vec4(center, 1.0f)};
...
// Frustrum createFrustrumFromCamera(const Camera& cam, float aspect, float fovY, float zNear, float zFar) {
// 	Frustrum frustrum;
// 	const float halfVSide = zFar * tanf(fovY * 0.5f);
// 	const float halfHSide = halfVSide * aspect;
// 	const glm::vec3 frontMultFar = zFar * cam.Front;

// 	frustrum.nearFace = {cam.Position + zNear * cam.Front, cam.Front};
// 	frustrum.farFace = {cam.Position + frontMultFar, -cam.Front};
// 	frustrum.rightFace = {cam.Position, glm::cross(frontMultFar - cam.Right * halfHSide, cam.Up)};
// 	frustrum.leftFace = {cam.Position, glm::cross(cam.Up, frontMultFar + cam.Right * halfHSide)};
// 	frustrum.topFace = {cam.Position, glm::cross(cam.Right, frontMultFar - cam.Up * halfVSide)};
// 	frustrum.bottomFace = {cam.Position, glm::cross(frontMultFar + cam.Up * halfVSide, cam.Right)};
// 	return frustrum;
// }
...
// 		const float newIk = std::abs(glm::dot(glm::vec3{ 0.0f, 0.0f, 1.0f }, right)) +
// 			std::abs(glm::dot(glm::vec3{ 0.0f, 0.0f, 1.0f }, up)) +
// 			std::abs(glm::dot(glm::vec3{ 0.0f, 0.0f, 1.0clf }, forward));
...
// 	void draw(const Frustrum& frustrum, Shader shader, unsigned int& display, unsigned int& total) {
// 		if (boundingVolume->isOnFrustrum(frustrum, transform)) {
...
// 		for(auto& child : children) {
// 			child->drawSelfAndChild(frustrum, shader, display, total);
// 		}
// 		// for (auto& child : children) {
//         // 	child->Draw(shader);
//     	// }
