...
int main() {
	...
	std::filesystem::path skyboxVertPath = projectRoot/"res"/"shaders"/"skybox.vert";
	...
	vector<std::filesystem::path> faces {
		projectRoot/"res"/"skybox"/"right.jpg",
		projectRoot/"res"/"skybox"/"left.jpg",
		projectRoot/"res"/"skybox"/"top.jpg",
		projectRoot/"res"/"skybox"/"bottom.jpg",
...
unsigned int loadCubemap(vector<std::filesystem::path> faces) {
	...
	for (unsigned int i = 0; i < faces.size(); i++) {
		...
		if (data) {
			...
			std::cout << "Cubemap texture failed to load at path: " << faces[i].string().c_str() << std::endl;
			stbi_image_free(data);
		}
	}
